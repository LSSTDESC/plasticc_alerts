...
mjd = [float(file[-12:-5]) for file in files]
#mjdnum = [float(d) for d in mjd]
#print(mjd)
hist, edges = np.histogram(mjd)
print(hist, edges)
for count,edge in enumerate(edges[0:-1]):
    
    high= edges[count+1]
    low = edges[count]
#     print(count, edge, low, high)
#     print('==========')
    inds = np.where((mjd < high) & (mjd >= low))[0]
#     print(inds)
    listfiles=[files[i] for i in inds]
#     print(listfiles)
    dir = "files_%5.1f"%edge
    try:
        os.mkdir(dir)
    except FileExistsError:
        pass
    [shutil.move(file,dir) for file in listfiles]
    

# files = glob.glob("*{}*.avro".format(name))
# print(files)
